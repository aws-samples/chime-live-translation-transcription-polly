...
const Transcription = (props: tTranscriptionProps) => {
    ...
    const [attendees, updateAttendees] = useState(new Map());
    ...
    function checkIfAttendeeExists(attendeeId: string) {
        return attendees.has(attendeeId);
    }

    function addAttendee(attendeeId: string, attendeeName: string) {
            ...
            <SpaceBetween size='xs'>
                <div style={{height: '663px', width: '240px'}} className={"transcriptionContainer"}>
                    {lines.map((line, index) => {
                        if (!checkIfAttendeeExists(line.attendeeName)) {
                            addAttendee(line.attendeeName, line.attendeeName);
                        }
                        //  Change color of text based on attendee
                        const currentColor = attendees.get(line.attendeeName).color;
                        return (
                            <div key={index} style={{color: currentColor}}>
